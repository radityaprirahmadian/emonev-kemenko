...
const GNRM = (props) => {
  const { resetDocument, editDocumentFalse, loading, setLoadingFalse, setLoadingTrue, preview } =
  ...
  const { limit, page, tahun, kp, instansi, totalDoc, keyword } = filter;
  ...
  const handleReset = () => {
    ...
    preview();
    history.push(
      `/${userDetail && userDetail.role === 'owner' ? 'super-admin' : 'admin'}/formulir-gnrm`,
    );
  };
        ...
        >
          <div className="toggle-laporan">
            <Link
              ...
              className={`item ${
                history.location.search.split('=')[1] === 'rencana-pelaksanaan-program'
                  ? 'active'
                  : ''
              }`}
            >
            ...
            <Link
              ...
              className={`item ${
                history.location.search.split('=')[1] === 'laporan-monev' ? 'active' : ''
              }`}
            >
              Laporan
            </Link>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <div className="input-dan-tajuk">
              <button className="tambah-program" onClick={() => handleReset()}>
                ...
                <h1 className="text-input-program">Input Program</h1>
              </button>
            </div>
          </div>
              ...
              <thead className="table-head">
                <tr>
                  ...
                  <th
                    ...
                    className={user && user.role === 'owner' ? '' : 'd-none'}
                  >
                    Instansi
                  </th>
                  <th
                    width={sidebar ? '160px' : '293px'}
                    style={{
                      transition: 'all 0.3s ease-in-out',
                    }}
                  >
