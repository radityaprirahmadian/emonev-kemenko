...
const GNRM = (props) => {
    ...
    const { limit, page, tahun, kp, instansi, totalDoc } = filter;
    ...
    const getAllDocument = async () => {
        ...
        try {
            const res = await axios.get(
                `https://api.simonev.revolusimental.go.id/api/v1/document?type=gnrm&tahun=${tahun}&instansi=${instansi}&limit=${limit}&page=${page}&kp=${kp}`,
                config
            );
