...
const FormReminder = (props) => {
  ...
  const { date, time, kepada, judul, isi } = reminder;

  const { nama_pendek } = instansi;
  ...
  const addNewNotification = async (formData) => {
    ...
    try {
      ...
      history.push(
        `/${userDetail && userDetail.role === 'owner' ? 'super-admin' : 'admin'}/notifikasi`,
      );
    } catch (err) {
      console.log(err);
    }
      ...
      {loading ? (
        ...
        >
          <div style={{ width: 'fit-content', height: 'fit-content', margin: 'auto' }}>
            <div
              className="reminder-1-container"
              style={
                sidebar
                  ? { width: '1050px', transition: 'all 0.3s ease-in-out' }
                  : { transition: 'all 0.3s ease-in-out' }
              }
            >
              <div className="asal-reminder" style={{ lineHeight: '20px' }}>
                Dari <br />
                <span className="nama-pengirim-reminder">{userDetail && userDetail.nama}</span>
                <br />
              ...
              {!sidebar ? (
                <form className="form-reminder-1" onSubmit={onSubmit}>
                  <div>
                    <div
                      className={
                        userDetail && userDetail.role === 'owner' ? 'div-reminder' : 'd-none'
                      }
                    >
                      <div className="row">
                        <div className="col-md-2">
                          <label
                            style={{ height: '42px', lineHeight: '42px', marginBottom: '16px' }}
                          >
                            Instansi Tujuan
                          </label>
                          <br />
                          ...
                          <div style={{ marginBottom: '16px' }}>
                            <div style={{ marginBottom: '16px' }}>
                              {userDetail && userDetail.role === 'owner' ? (
                                <select
                                  className="reminder-tujuan"
                                  type="text"
                                  required
                                  name="nama_pendek"
                                  onChange={(e) =>
                                    onChangeDropDown(e, 'pengguna0', 'tujuan0', 'instansi0')
                                  }
                                >
                            ...
                            <div style={{ marginBottom: '16px' }}>
                              <select
                                className="reminder-akun"
                                type="text"
                                required
                                onChange={(e) => onChangeTujuan(e, 'tujuan0')}
                              >
                                <option defaultValue="" hidden></option>
                                {userNew &&
                                  userNew.pengguna0 &&
                                  userNew.pengguna0.map((user) => {
                                ...
                                {userNew && !userNew[`pengguna0`] && (
                                  <option>{'Pilih instansi terlebih dahulu'}</option>
                                )}
                              </select>
                            </div>
                          </div>
                              ...
                              <div style={{ marginBottom: '16px' }} key={index}>
                                  ...
                                  {userDetail && userDetail.role === 'owner' ? (
                                    <Fragment>
                                      ...
                                      >
                                        ...
                                        {allInstansi &&
                                          allInstansi.map((instansi, i) => {
                                            let selected = false;
                                            for (let i = 0; i < allInstansi.length; i++) {
                                              selected =
                                                instansi ===
                                                Object.values(selectedInstansi)[index + 1];
                                            }
                                            return (
                                              <option key={i} selected={selected} value={instansi}>
                                                {instansi}
                                              </option>
                                            );
                                          })}
                                      ...
                                      >
                                        <i className="" onClick={() => onDeleteTujuan(index + 1)}>
                                          {' '}
                                          x{' '}
                                        </i>
                                      </span>
                                ...
                                <div style={{ marginBottom: '16px' }}>
                                  <select
                                    ...
                                    type="text"
                                    required
                                    onChange={(e) => onChangeTujuan(e, `tujuan${index + 1}`)}
                                  >
                                    <option defaultValue="" hidden></option>
                                    {userNew &&
                                      userNew[`pengguna${index + 1}`] &&
                                      userNew[`pengguna${index + 1}`].map((user) => {
                                        let alreadySelected = false;
                                        alreadySelectedUser.forEach((user1) => {
                                        ...
                                        return (
                                          <option
                                            ...
                                            name="kepada"
                                            selected={
                                              user._id === selectedUser[`tujuan${index + 1}`]
                                                ? true
                                                : false
                                            }
                                            hidden={alreadySelected}
                                    ...
                                    {userNew && !userNew[`pengguna${index + 1}`] && (
                                      <option>{'Pilih instansi terlebih dahulu'}</option>
                                    )}
                                  </select>
                                </div>
                  ...
                  {form.length < totalUsers - 1 ? (
                    <div style={{ height: '30px', marginBottom: '8px' }}>
                      <label className="tambah-lembaga">Tambah Tujuan</label>
                      <img
                        src={plus2}
                        style={{
                          position: 'absolute',
                  ...
                  <div className="reminder-navigation-button">
                    <button className="button-kirim" type="submit">
                      KIRIM
                    </button>
                  </div>
                </form>
                        ...
                        >
                          <div style={{ marginBottom: '16px' }}>
                            <div style={{ marginBottom: '16px' }}>
                              {userDetail && userDetail.role === 'owner' ? (
                                <select
                                  className="reminder-tujuan-test"
                                  type="text"
                                  required
                                  name="nama_pendek"
                                  onChange={(e) =>
