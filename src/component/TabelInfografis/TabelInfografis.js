...
const TabelInfografis = (props) => {
    ...
    const onOpen = (e) => {
        e.preventDefault()
        setOpen(true)
        const gams = props.gambar.map(gambar => `https://api.simonev.revolusimental.go.id${gambar.path}` )
        setGambar(gams)
    }
        ...
        <Fragment>
            {
                ...
                <div style={{position: 'fixed' ,top: '0' ,bottom: '0' ,left: '0', right: '0' , zIndex: '9998' ,backgroundColor: 'rgba(0,0,0,0.4)'}}>
                    <div className="popup_delete" style={{width:'400px',height:'300px', borderRadius:'10px', padding:'28px', zIndex:'9998', backgroundColor:'white', position:'fixed',top:'20%',left:'40%'}}>
                        ...
                        <h1 style={{fontSize:'18px', textAlign:'center' , fontWeight:'normal', lineHeight:'20px'}}>Apakah anda yakin akan menghapus <br/> kabar GNRM ini?</h1>
